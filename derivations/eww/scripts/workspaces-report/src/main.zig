...
const MonitorsState = struct {
    ...
    pub fn addMonitor(
        ...
        try newMonitor.workspaces.appendSlice(self.alloc, &.{
            .{ .name = "1" },
            .{ .name = "2" },
            .{ .name = "3" },
            .{ .name = "4" },
        ...
        try self.monitors.append(self.alloc, newMonitor);
        if (focused) {
    ...
    pub fn format(self: *const @This(), stdout: *std.Io.Writer) !void {
        try stdout.print("{f}", .{std.json.fmt(
            self.monitors.items,
            .{ .whitespace = .minified },
        )});
